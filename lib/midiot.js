...
MIDIoT.prototype.bind = function(options) {
  ...
  if(options.barnacles) {
    ...
    self.notifications.on('appearance', function(event) {
      handleNotification(self, 'appearance', event.tiraid);
    });
    self.notifications.on('keep-alive', function(event) {
      handleNotification(self, 'keep-alive', event.tiraid);
    });
    self.notifications.on('displacement', function(event) {
      handleNotification(self, 'displacement', event.tiraid);
    });
    self.notifications.on('disappearance', function(event) {
      handleNotification(self, 'disappearance', event.tiraid);
    });
  }
}
...
function handleNotification(instance, type, tiraid) {
  instance.devicesManager.handleEvent(type, tiraid, function(status) {
    ...
    else if(instance.socket) { // No MIDI event, but send on socket anyway
...
function produceMessage(instance, status) { // TODO: choose better names
  ...
  else if(status.category === 'Controller') {
...
function getMessageCodes(type, options) {
  switch(type) {
    ...
    case "controlChange":
      ...
      var data1 = (options.key % 127) || DEFAULT_NOTE;          // Number
      var data2 = (options.velocity % 127) || DEFAULT_VELOCITY; // Value
      return [status, data1, data2];
    default:
