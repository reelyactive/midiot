...
function MIDIoT(options) {
  ...
  self.oscLocalAddress = options.oscAddress || DEFAULT_OSC_ADDRESS;
  self.oscLocalPort = options.oscPort || DEFAULT_OSC_PORT;
...
function sendOscMessage(instance, message) {
  var oscArgs = Uint8Array(message.splice(0, 0, DEFAULT_PORT_ID));
  var midiMessage = { address: "/midiMessage", args: [ oscArgs ] };
