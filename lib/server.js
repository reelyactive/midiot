...
function MIDIoT(options) {
  ...
  this.middleware = new barnowl( { n: 3, enableMixing: true } );
  this.notifications = new barnacles();
  ...
  this.notifications.on('appearance', function(tiraid) {
    handleNotification(self, 'appearance', tiraid);
  });
  this.notifications.on('displacement', function(tiraid) {
    handleNotification(self, 'displacement', tiraid);
  });
  this.notifications.on('disappearance', function(tiraid) {
...
MIDIoT.prototype.bind = function(options) {
  ...
  if(options.barnowl) {
    self.middleware.bind(options.barnowl);
  }    
}
